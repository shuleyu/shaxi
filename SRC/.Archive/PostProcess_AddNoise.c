...
//     enum PIenum {};
    enum PSenum {infile,noisefile};
    enum Penum  {Period,Length,delta,NoiseLevel};
...
//     return 1;
    ...
    ****************************************************************/
	...
	// Read in Noise.
	int maxl=2000000,rawnpts;
	float *noise=(float *)malloc(maxl*sizeof(float));
	float rawbeg,rawdel;
	int *random_begin=(int *)malloc(nptsx*sizeof(int));
	rsac1(PS[noisefile],noise,&rawnpts,&rawbeg,&rawdel,&maxl,&nerr,strlen(PS[noisefile]));
	retrend(0.0,noise,rawnpts,rawbeg);
	normalize(noise,rawnpts);
	random_int(0,rawnpts-1,random_begin,nptsx);
	...
	for (Cnt=0;Cnt<nptsx;++Cnt){
		for (Cnt2=0;Cnt2<nptsy;++Cnt2){
			tmpdata[Cnt2]=data[Cnt][Cnt2]+noise[(random_begin[Cnt]+Cnt2)%rawnpts]*P[NoiseLevel]*4.0/10000000;
		}
// 		retrend(0.0,tmpdata,nptsy,del_float);
